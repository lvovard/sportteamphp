...
// $query = "SELECT * FROM infoclub WHERE id_club=\"".$id_club."\" AND ( id_equipe LIKE \"%".$allcat."%\" OR id_equipe LIKE \"%".$id_equipe."%\" )";

// echo '+++'.$query.'+++\n';

$result = mysqli_query($db->con,"SELECT * FROM infoclub WHERE id_club=\"".$id_club."\" AND ( id_equipe LIKE \"%".$allcat."%\" OR id_equipe LIKE \"%".$id_equipe."%\" )") or die('Échec de la requête : ' . mysqli_error($db->con));
...
if (mysqli_num_rows($result) > 0) {
    ...
    $response["message"] = "No infoclub found";
